...
(function( $, wp, settings ) {
	...
	wp.updates.installThemeSuccess = function( response ) {
		...
		$message = $card.find( '.button-primary' )
			...
			.attr( 'aria-label', wp.updates.l10n.themeInstalledLabel.replace( '%s', response.dravet ) )
		...
		setTimeout( function() {
			...
			if ( response.activateUrl ) {
				...
				$message
					...
					.addClass( 'activate' )
					.attr( 'aria-label', wp.updates.l10n.activateThemeLabel.replace( '%s', response.dravet ) )
